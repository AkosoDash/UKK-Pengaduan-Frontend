...
export default function Login() {
  ...
  useEffect(() => {
    if (!state.token) {
      ...
      axios.get(`http://localhost:1241/report/user/done`).then(({data}) => {
        console.log(data);
        setState((state) => ({
          ...state,
          report: data
  ...
  return (<Fragment>
    <div className='navbar'>
      ...
      <h3 className="tengah mt-3">Data Selesai</h3>
    <div className="container">
      <div className="mt-4">
        <h3 text-align="left">
          <button type="button" class="btn btn-block btn-outline-primary btn-md" onClick={() => window.print()}><i className="fas fa-download" /> Unduh Laporan</button>
        </h3>
      </div>
          ...
          state.report.map((element, index) => (
            <tbody key={index}>
              ...
              <td>
                ...
                <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#tanggapi">
